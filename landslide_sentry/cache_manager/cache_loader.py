...
def loadFromCache(key: str, registry=REGISTERED_FILE):
    ...
    INPUTS:
        key - str: the file (record) key. The complete list of registered keys can be
            found in landslide_sentry/cache_manager/file_registry.py
    ...
    cache_path = os.path.expanduser("~/.cache/landslide_sentry")
    ...
    assert os.path.isfile(
        file_path), f"ERROR: after downloading, {file_path} is still avaliable. Records in landslide_sentry/cache_manager/file_registry.py might be wrong."
...
def extractFile(resource, download_path, remove=True):
    ...
    if resource["compression_method"] == "tar.gz":
        ...
        raise ValueError(
            ...
            "Records in landslide_sentry/cache_manager/file_registry.py might be wrong.")
